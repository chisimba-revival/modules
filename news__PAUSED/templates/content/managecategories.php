...
$objIcon->alt = $this->objLanguage->languageText('mod_news_editcategory', 'news', 'Edit Category');
$objIcon->title = $this->objLanguage->languageText('mod_news_editcategory', 'news', 'Edit Category');
$editIcon = $objIcon->show();

$objIcon->setIcon('delete');
$objIcon->alt = $this->objLanguage->languageText('phrase_deletecategory', 'news', 'Delete Category');
$objIcon->title = $this->objLanguage->languageText('phrase_deletecategory', 'news', 'Delete Category');
$deleteIcon = $objIcon->show();

$header = new htmlheading();
$header->type = 1;
$header->str = $this->objLanguage->languageText('mod_news_newsmenu', 'news', 'News Menu');
echo $header->show();
...
if ((is_countable($menuItems) ? count($menuItems) : 0) == 0) {
    echo '<div class="noRecordsMessage">'.$this->objLanguage->languageText('mod_news_nomenuitemssetup', 'news', 'No Menu Items have been setup yet.').'</div>';
} else {
    ...
    $table->startHeaderRow();
        ...
        $table->addHeaderCell($this->objLanguage->languageText('mod_news_menuitemname', 'news', 'Menu Item Name'));
        $table->addHeaderCell($this->objLanguage->languageText('word_type', 'word', 'Type'));
        $table->addHeaderCell($this->objLanguage->languageText('mod_news_examplelink', 'news', 'Example Link'));
    ...
    foreach ($menuItems as $item)
    {
        ...
        switch ($item['itemtype'])
        {
            case 'divider':
                $edit = '&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;';
                $delete = $objIcon->getDeleteIconWithConfirm($item['id'], array('action'=>'deletedivider','id'=>$item['id']), 'news', $this->objLanguage->languageText('mod_news_confirmdeletedivider', 'news', 'Are you sure want to delete this divider?'));
                $numItems = '&nbsp;';
                break;
            case 'text':
                ...
                $delete = $objIcon->getDeleteIconWithConfirm($item['id'], array('action'=>'deletetext','id'=>$item['id']), 'news', $this->objLanguage->languageText('mod_news_confirmdeletetext', 'news', 'Are you sure want to delete this text?'));
                $numItems = '&nbsp;';
                break;
            case 'module':
                $edit = '&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;';
            ...
            default:
                ...
                if ($numItems == 0) {
                    ...
                    $link = new link ('javascript:alert(\''.addslashes($this->objLanguage->languageText('mod_news_cannotdeletecategorywithstories', 'news', 'Cannot delete category because it contains stories')).'\');');
                    $link->link = $deleteIcon;
...
$link->link = $this->objLanguage->languageText('mod_news_addnewmenuitem', 'news', 'Add New Menu Item');
