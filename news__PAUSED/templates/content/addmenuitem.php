...
$header->type = 1;
$header->str = $this->objLanguage->languageText('mod_news_addmenuitem', 'news', 'Add Menu Item');

echo $header->show();

$objCache = $this->getObject('cache', 'utilities');
$objCache->setup('addmenuitemform', 'news', 1000);

// Add Divider Form 

$objCache->setup('adddividerform', 'news', 1000);
...
if ($addDividerForm == FALSE)  {
    ...
    $button = new button ('adddividerbutton', $this->objLanguage->languageText('mod_news_adddividingruler', 'news', 'Add Dividing Ruler to Menu'));
    $button->setToSubmit();
...
$objCache->setup('addurlform', 'news', 1000);
...
$objCache->setup('addtextform', 'news', 1000);
...
$switchmenu->addBlock($this->objLanguage->languageText('mod_news_addlinktomenu', 'news', 'Add Link to Module'), $addModuleFormStr); 
