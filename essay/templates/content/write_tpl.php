...
async function save(){if(!dirty||saving)return true;saving=true;const savingRevision=revision;state.textContent='Saving…';const body=new URLSearchParams({csrf_token:token,body_html:editor.innerHTML});try{const response=await fetch(saveUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','Accept':'application/json'},body:body.toString(),credentials:'same-origin'});const data=await response.json();token=data.csrf||token;if(!data.ok)throw new Error('save failed');dirty=revision!==savingRevision;state.textContent=dirty?'Unsaved changes':'Saved';if(dirty){clearTimeout(timer);timer=setTimeout(save,500);}return true;}catch(error){dirty=true;state.textContent='Could not save — retrying';return false;}finally{saving=false;}}
