...
$controller=$read('controller.php');$graph=$read('classes/knowledgemapgraphservice_class_inc.php');$importer=$read('classes/knowledgemapimportservice_class_inc.php');$authorization=$read('classes/knowledgemapauthorizationservice_class_inc.php');$template=$read('templates/content/view_tpl.php');$index=$read('templates/content/index_tpl.php');$css=$read('resources/knowledgemap.css');
$checks=array(
    ...
    'knowledge tables preserve full Unicode'=>count(array_filter(array('sql/tbl_knowledgemap_maps.sql','sql/tbl_knowledgemap_nodes.sql','sql/tbl_knowledgemap_relationships.sql','sql/tbl_knowledgemap_access.sql'),fn($file)=>str_contains($read($file),"'charset' => 'utf8mb4'")&&str_contains($read($file),'utf8mb4_unicode_ci')))===4,
    ...
    'node collapse uses bare skin icon primitive'=>str_contains($read('resources/knowledgemap.js'),'chisimba-icon-button--bare knowmap-node__collapse'),
    ...
    'workspace bounds include every side of large maps'=>str_contains($read('resources/knowledgemap.js'),'prototype.normaliseWorkspace')&&str_contains($read('resources/knowledgemap.js'),'padding-minY')&&str_contains($read('resources/knowledgemap.js'),'this.worldHeight')&&!str_contains($css,'width: 2400px'),
    ...
    'selected colour applies to the complete containment branch'=>str_contains($template,'data-knowmap-action="apply-branch-colour"')&&str_contains($read('resources/knowledgemap.js'),'applyBranchColour')&&str_contains($read('resources/knowledgemap.js'),'concat(this.descendants(id))'),
    'map cards use compliant icon-labelled skin actions'=>str_contains($index,'chisimba-button-secondary chisimba-button-compact')&&str_contains($index,"render('arrow-right'")&&str_contains($index,'<span>Open map</span>')&&!str_contains($index,'chisimba-navigation-action'),
    'branch lines remain strong when the map is zoomed'=>str_contains($css,'vector-effect: non-scaling-stroke')&&str_contains($read('resources/knowledgemap.js'),'branchLineColour'),
