...
{
    /** List the current user's icon images in the dedicated km-icons folder. */
    public function choices(){
        ...
        return array_values(array_filter($result['files'],fn($file)=>$this->isIconImage($file)));
    }
    ...
    /** Restrict custom icon content to supported image formats; SVG is loaded only as an image. */
    public function isIconImage(array $file){
        return in_array(strtolower((string)($file['mimetype']??'')),array('image/png','image/jpeg','image/gif','image/webp','image/bmp','image/svg+xml'),true)
    ...
    public function markup(array $file){
        if(!$this->isIconImage($file))return '';
    ...
    public function library(array $document){
        ...
        foreach($document['nodes']??array() as $node){
            ...
            if(!is_array($file)||!$this->isIconImage($file)||!preg_match('~^users/([A-Za-z0-9_-]+)/km-icons$~',(string)($file['filefolder']??''),$owner))continue;
