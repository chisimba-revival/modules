...
class knowledgemapgraphservice extends controller
{
    ...
    public function normalise(array $document){
        ...
        foreach((array)($document['nodes']??array()) as $index=>$node){$id=$this->identifier($node['id']??'');if($id===''||isset($seenNodes[$id])){$errors[]='Node '.($index+1).' has a missing or duplicate identifier.';continue;}$seenNodes[$id]=true;$nodes[]=array('id'=>$id,'type'=>$this->nodeType($node['type']??'standard'),'title'=>$this->text($node['title']??'Untitled',255),'description'=>$this->text($node['description']??'',100000),'presentation'=>$this->presentation($node),'order'=>$this->sortOrder($node['order']??$index,$index));}
    ...
    private function presentation(array $node){$allowed=array('color','icon','fontColor','fontSize','x','y','side','dir','collapsed','fontFamily');$source=is_array($node['presentation']??null)?array_merge($node,$node['presentation']):$node;$out=array();foreach($allowed as $key)if(isset($source[$key])&&(is_scalar($source[$key])||is_bool($source[$key])))$out[$key]=$source[$key];foreach(array('offsetX','offsetY') as $key)if(isset($source[$key])&&is_numeric($source[$key])&&is_finite((float)$source[$key]))$out[$key]=max(-100000,min(100000,(float)$source[$key]));if(in_array($source['mediaIntent']??'',array('create-video','use-video','create-text','use-text'),true))$out['mediaIntent']=$source['mediaIntent'];if(isset($source['branchGap'])&&is_numeric($source['branchGap'])&&is_finite((float)$source['branchGap']))$out['branchGap']=max(0,min(2000,(float)$source['branchGap']));return $out;}
    ...
    /** Keep persisted ordering inside the portable signed 32-bit schema range. */
    private function sortOrder($value,$fallback){return is_numeric($value)&&(float)$value>=0&&(float)$value<=2147483647?(int)$value:(int)$fallback;}
