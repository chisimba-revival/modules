...
foreach(['teacher','other','student'] as $role){$id=$users->generateUserId();$name=$tag.$role;$result=$provision->createLocalUser(['userId'=>$id,'username'=>$name,'firstName'=>'Workshop QA','surname'=>$role,'emailAddress'=>$name.'@example.invalid','isActive'=>1,'howCreated'=>'questionworkshop-fixture'],$password);if(empty($result['ok']))throw new RuntimeException($result['code']);if($role!=='student')$group->ensureMembership($group->groupIdForName('Lecturers'),$e->getObject('identityservice','security')->ensurePermissionIdentity($id));$f['users'][$role]=$name;$f['ids'][$role]=$id;}
