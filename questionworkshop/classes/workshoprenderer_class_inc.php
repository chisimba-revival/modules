...
class workshoprenderer extends ChisimbaObject
{
    ...
    {
        return 'data-workshop-form data-token-url="'.self::escape($this->uri(['action'=>'formtoken'],'questionworkshop')).'" data-session-error="'.self::escape($this->text('session_unavailable')).'"';
    }
    public function form($action,$token,$id=''){return '<form '.$this->formAttributes().' method="post" enctype="multipart/form-data"'.($action==='generate'?' data-workshop-generate data-pending="'.self::escape($this->text('generating')).'"':'').' action="'.self::escape($this->uri(['action'=>$action,'id'=>$id],'questionworkshop')).'"><input type="hidden" name="csrf_token" value="'.self::escape($token).'">';}
}
