...
$header->type = 1;
$header->str = $this->objLanguage->languageText("mod_userdetails_updatedetails", 'userdetails', "Update Details");
...
$form->addToForm($fieldset->show());
$form->addToForm('<br />');
$button = new button('submitform', 'Update Registration');
$button->setToSubmit();
$form->addToForm('<p align="center"><br />' . $button->show() . '</p><br/ ><br/ >');
if ($mode == 'addfixup') {
    if (!empty($results)) {
        foreach($results as $result) {
            $messages[] = $this->explainProblemsInfo($result);
        }
    }
}
