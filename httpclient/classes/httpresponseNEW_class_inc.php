...
class httpresponse
{
     ...
     *
     * In most cases, you would use httpresponse::factory to parse an HTTP
    ...
    public function __construct($code, $headers, $body = null, $version = '1.1', $message = null)
    {
        ...
        if (is_null(self::responseCodeAsText($code)))
            throw new customException("{$code} is not a valid HTTP response code");
        ...
        if (! preg_match('|^\d\.\d$|', $version))
            throw new customException("Invalid HTTP response version: $version");
    ...
    static public function decodeChunkedBody($body)
    {
        ...
        while (trim($body)) {
            if (! preg_match("/^([\da-fA-F]+)[^\r\n]*\r\n/sm", $body, $m)) {
                throw new customException("Error parsing body - doesn't seem to be a chunked message");
            }
