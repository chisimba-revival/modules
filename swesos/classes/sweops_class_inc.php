 ...
 * @access public
 */
class sweops extends ChisimbaObject
{
	public $objConfig;
    ...
    public function uploadDataFile($featurebox = TRUE)
    {
        ...
        //file textfield
        $fileadd->startRow();
        $filelabel = new label($this->objLanguage->languageText('mod_swe_file', 'swesos') .':', 'input_file');
        
        $objSelectFile->name = 'swefile';
        $objSelectFile->restrictFileList = array('txt');
        ...
        if ($featurebox == TRUE) {
            ...
            $ret = $objFeatureBox->showContent($this->objLanguage->languageText("mod_swe_uploadfile", "swesos") , $fileform);
            return $ret;
        } else {
            return $fileform;
        }
