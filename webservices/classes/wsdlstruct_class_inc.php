...
class wsdlstruct {
	...
	public function __construct($tns, $url, $type = SOAP_RPC, $use = SOAP_ENCODED){
		...
		$this->xmlSchema = new genxmlschema($this->xsdSchema);
	 ...
	 * @return void
	 */
	public function setService(genreflectionclass $class){
	...
	public function generateDocument(){
		...
		foreach((array)$this->services as $serviceName => $service){
			...
			foreach((array)$service->methods as $operation){
				...
				if(!$operation->return || trim($operation->return) == "") throw new customException('No return type for '.$operationName);
	...
	private function addMessage($name, $parts){
		...
		foreach((array)$parts as $partName => $partType){
			...
			if($t = genxmlschema::checkSchemaType(strtolower($partType)))
