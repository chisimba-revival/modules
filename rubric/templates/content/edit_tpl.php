...
$url = function ($params, $module = 'rubric') { return $this->uriForHtmlAttribute($params, $module); };
...
$action = $url(array(
    'action'=>'edittableconfirm', 'tableId'=>$tableId,
...
echo '</ol></details><form class="rubric-editor-form" method="post" action="'.$action.'">'
...
echo '</tbody></table></div><div class="rubric-editor-actions"><button class="button" type="submit">'
    ...
    .'<a class="button chisimba-button-secondary" href="'.$url(array('action'=>'viewtable','tableId'=>$tableId)).'">'
...
if (!isset($suppressModify)) {
    echo '<nav class="rubric-structure-actions" aria-label="'.$esc($objLanguage->languageText('mod_rubric_structure_actions', 'rubric')).'">'
        .'<a class="button chisimba-button-secondary" href="'.$url(array('action'=>'addrow','tableId'=>$tableId)).'">'.$icon('plus').'<span>'.$esc($objLanguage->languageText('rubric_add_row', 'rubric')).'</span></a>'
        .'<a class="button chisimba-button-secondary" href="'.$url(array('action'=>'addcol','tableId'=>$tableId)).'">'.$icon('plus').'<span>'.$esc($objLanguage->languageText('rubric_add_column', 'rubric')).'</span></a>';
    if ($rows > 1) {
        echo '<a class="button rubric-button-danger" href="'.$url(array('action'=>'delrow','tableId'=>$tableId)).'">'.$icon('minus').'<span>'.$esc($objLanguage->languageText('rubric_delete_row', 'rubric')).'</span></a>';
    }
    if ($cols > 1) {
        echo '<a class="button rubric-button-danger" href="'.$url(array('action'=>'delcol','tableId'=>$tableId)).'">'.$icon('minus').'<span>'.$esc($objLanguage->languageText('rubric_delete_column', 'rubric')).'</span></a>';
    }
    echo '</nav>';
}
?>
