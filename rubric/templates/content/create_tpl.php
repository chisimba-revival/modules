...
$action = $url(array(
    'action'=>'createtableconfirm', 'type'=>$_type,
...
$cancelUrl = ($returnModule === 'worksheet' && in_array($returnAction, array('editquestion', 'managequestions')) && $returnId !== '')
    ? $url(array('action'=>$returnAction, 'id'=>$returnId), 'worksheet')
    : $url(array());
...
echo '</select></div></div><div class="rubric-form-actions"><button class="button" type="submit">'.$icon('plus').'<span>'
    ...
    .'<a class="button chisimba-button-secondary" href="'.$cancelUrl.'">'.$icon('x').'<span>'.$esc($objLanguage->languageText('word_cancel')).'</span></a>'
    .'</div></form></main>';
?>
