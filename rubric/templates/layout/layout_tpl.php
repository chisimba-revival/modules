...
$ret = $this->getContent();
$scopeBar = '';
if (isset($currentScope) && $currentScope !== '') {
    $scopeBar = '<div class="rubric-scope-bar"><span><strong>'
        .$this->objLanguage->languageText('rubric_current_scope', 'rubric')
        .':</strong> '.htmlspecialchars($currentScope, ENT_QUOTES, 'UTF-8').'</span>';
    if (isset($rubricScope) && $rubricScope !== '') {
        $scopeBar .= '<span><strong>'.$this->objLanguage->languageText('rubric_resource_scope', 'rubric')
            .':</strong> '.htmlspecialchars($rubricScope, ENT_QUOTES, 'UTF-8').'</span>';
    }
    $scopeBar .= '</div>';
}
$ret = $scopeBar.$ret;
