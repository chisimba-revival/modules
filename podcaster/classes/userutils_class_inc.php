...
/**
 * This class contains utilities for doing common functions in the podcaster module
 *  PHP version 5
 *
...
class userutils extends ChisimbaObject {
    ...
    public function saveFile($path, $docname, $docid) {
        ...
        $objFileUpload = $this->getObject('podcastupload');
    ...
    public function deleteFile($userid, $id) {
        ...
        if ($permission == 1) {
            $myFile = $this->objConfig->getcontentBasePath() . '/podcasterUploadFiles/' . $userid . '/shared/' . $filename;
        } else {
            $myFile = $this->objConfig->getcontentBasePath() . '/podcasterUploadFiles/' . $userid . '/' . $filename;
        }
    ...
    function getFiles($node, $limit=Null, $rowcount=Null) {
        ...
        foreach ($rows as $row) {
            ...
            $files[] = array(
                'text' => '<img src="' . $this->sitePath . '/podcaster/resources/images/ext/' . $this->findexts($row['filename']) . '-16x16.png">&nbsp;' . $row['filename'],
                ...
                'filesize' => $size,
                'thumbnailpath' => '<img src="' . $this->sitePath . '/podcaster/resources/images/ext/' . $this->findexts($row['filename']) . '.png" width="22" height="22">',
            );
        }
    ...
    function getFolders($mode, $node="") {
        $objUser = $this->getObject("user", "security");
        $dir = $this->objSysConfig->getValue('FILES_DIR', 'podcaster');
        $apodir = $this->objSysConfig->getValue('APO_DIR', 'podcaster');
        $this->objUser = $this->getObject('user', 'security');
        $size = "";
    ...
    function downloadFile($filepath, $filename) {
        ...
        $baseDir = $this->objSysConfig->getValue('FILES_DIR', 'podcaster');
     ...
     * @return boolean
     */
    public function defaultFolderExistsCheck() {
    ...
    public function createfolder($folderpath, $foldername) {
        $this->objMkdir = $this->getObject('mkdir', 'files');
        $userid = $this->getUserId();
        $path = $this->objSysConfig->getValue('FILES_DIR', 'podcaster') . '/' . $userid . '/' . $folderpath . '/' . $foldername;
        $result = $this->objMkdir->mkdirs($path);
        $userid = $this->getUserId();
        $fullpath = $folderpath . '/' . $foldername;
        //Search and replace double //
    ...
    public function renamefolder($folderpath, $foldername) {
        ...
        $userid = $this->getUserId();
        $prevpath = $this->objSysConfig->getValue('FILES_DIR', 'podcaster') . '/' . $userid . '/' . $folderpath;
        $newpath = $this->objSysConfig->getValue('FILES_DIR', 'podcaster') . '/' . $userid . '/' . $foldername;
    ...
    public function deleteFolder($folderpath) {
        $folderpath = str_replace("//", "", $folderpath);
        $userid = $this->getUserId();
        $fullpath = $this->objSysConfig->getValue('FILES_DIR', 'podcaster') . '/' . $userid . '/' . $folderpath;
    ...
    function getRefNo($id) {
        $objDocuments = $this->getObject('dbdocuments', 'podcaster');
    ...
    function listdiraux($dir, &$files) {
        $userId = $this->getUserId();
        $doubleUserId = $userId . '/' . $userId;
        //$dir = str_replace($doubleUserId, $userId, $dir);
        ...
        while (($file = readdir($handle)) !== false) {
            ...
            if (is_dir($filepath)) {
                ...
                $cfile = substr($filepath, strlen($fpath));
                $pfile = substr($cfile, strlen($this->getUserId()));
                if ($this->folderPermissions->isValidFolder($pfile)) {
                    $files[] = $cfile;
                }
    ...
    function getTree($treeType='dhtml', $selected='', $treeMode='side', $action='') {
        ...
        $podcasts = $this->getObject('dbdocuments');
        $count = $podcasts->getUnapprovedDocsCount();
        if ($treeMode == 'side') {
            $unapprovedDocs = "$count New podcasts";
            ...
            $count = $podcasts->getRejectedDocsCount();
            $rejectedDocs = "$count Rejected podcasts";
            if ($selected == 'rejectedpodcasts') {
                $rejectedDocs = '<strong>' . $rejectedDocs . '</strong>';
                $cssClass = 'confirm';
            } else {
                $cssClass = '';
            }
            $rejectedDocsNode = new treenode(array('text' => $rejectedDocs, 'link' => $this->uri(array('action' => 'rejectedpodcasts', 'folder' => $baseFolderId)), 'icon' => $icon, 'expandedIcon' => $expandedIcon, 'cssClass' => $cssClass));
...
//Create a new tree
        ...
        $refArray = array();
        $refArray[$this->getUserId()] = & $allFilesNode;
    ...
    function getManageTree($treeType='dhtml', $selected='', $treeMode='side', $action='') {
        ...
        $baseFolderId = "0";
        $objfolders = $this->getObject('dbfolderpermissions');
        //Add manage folders node
        if ($treeType == 'htmldropdown') {
            $manageNode = new treenode(array('text' => $this->objLanguage->languageText('mod_podcaster_managefolders', 'podcaster', "Manage folders"), 'link' => ""));
        } else {
            $manageNode = new treenode(array('text' => $this->objLanguage->languageText('mod_podcaster_managefolders', 'podcaster', "Manage folders"), 'link' => "", 'icon' => $icon, 'expandedIcon' => $expandedIcon, 'cssClass' => $cssClass));
        }
        if ($treeMode == 'side') {
            $createfolders = $this->objLanguage->languageText('mod_podcaster_addfolder', 'podcaster', "Add folder");
            ...
            $removefolder = $this->objLanguage->languageText('mod_podcaster_deletefolder', 'podcaster', "Delete folder");
    ...
    function showCreateFolderForm($name='', $formaction = 'createfolder',$labelText="Create", $buttonLabel = "", $note="", $buttonNote="") {
        ...
        $form->addToForm("<br />" . $labelText . " in " . $this->getTree('htmldropdown'));
        
        if ($note != "") {
            $form->addToForm(' &nbsp; * ' . $label->show() . $textinput->show());
            $form->addToForm("<br /><br />" . $note);
        } else {
            $form->addToForm(' &nbsp; ' . $label->show() . $textinput->show());
        }
    ...
    /*
     * Function to generate a form that allows one to delete a podcaster folder/topic
    ...
    function showDeleteFolderForm($name='', $message=Null) {
        ...
        $form->addToForm("<br/>" . $this->objLanguage->languageText('mod_podcaster_selectdeletefolder', 'podcaster', "Select the folder you want to delete") . " " . $this->getTree('htmldropdown'));

        $button = new button('submit', $this->objLanguage->languageText('mod_podcaster_deletefolder', 'podcaster', "Delete a folder"));
        $button->cssId = 'savebutton';
        $button->setToSubmit();
        ...
        $fs = new fieldset();
        $fs->setLegend($this->objLanguage->languageText('mod_podcaster_deletetopic', 'podcaster', "Delete a topic"));
        $fs->addContent($form->show());
        return $fs->show();
    }
