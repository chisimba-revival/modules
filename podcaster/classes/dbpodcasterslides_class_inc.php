...
class dbpodcasterslides extends dbtable
{
    ...
    */
    ...
    {
        parent::init('tbl_podcaster_slides');
    ...
    {
        // Setup Directory
        $dir = $this->objConfig->getcontentBasePath().'podcaster/'.$id;
    ...
    {
        $full = $this->objConfig->getcontentBasePath().'podcaster_slide_thumbnails/'.$slideId.'.jpg';
        $rel = $this->objConfig->getsiteRoot().$this->objConfig->getcontentPath().'podcaster_slide_thumbnails/'.$slideId.'.jpg';
    ...
    private function generateSlideThumbnail($fileId, $slideId, $order)
    {
        ...
        $destination = $this->objConfig->getcontentBasePath().'podcaster_slide_thumbnails/'.$slideId.'.jpg';
        ...
        if (file_exists($destination)) {
            ...
            // Get Source Image
            $source = $this->objConfig->getcontentBasePath().'podcaster/'.$fileId.'/img'.($order-1).'.jpg';
            ...
            if (file_exists($source)) {
                ...
                $destinationDir = $this->objConfig->getcontentBasePath().'/podcaster_slide_thumbnails';
    ...
    public function getPodcastView($id)
    {
        ...
        $podpath = str_replace('&', '&amp;', $objFile->getFilePath($id));
        //$podpath = "usrfiles/podcaster/".$id."/".$id."/.mp3";
        $soundFile = str_replace(' ', '%20', $podpath);
        $objSoundPlayer->setSoundFile($soundFile);
        
        $slidesContent .= '<li>'.$objSoundPlayer->show().'</li></ul>';


        return array('slides'=>$slidesContent, 'transcript'=>'', 'slideshow'=>$slidesContent);
    }
    ...
    public function deleteSlides($fileId)
    {
        ...
        if ((is_countable($slides) ? count($slides) : 0) > 0)
        {
            foreach ($slides as $slide)
            {
                ...
                $firstPage =  $this->objConfig->getcontentBasePath().'podcaster/'.$fileId.'/'.$fileId.'.html';
                if (file_exists($firstPage))
                {
                    unlink($firstPage);
                }

                $img = $this->objConfig->getcontentBasePath().'podcaster/'.$fileId.'/img'.($slide['slideorder']-1).'.jpg';
                if (file_exists($img))
                {
                    unlink($img);
                }

                $html = $this->objConfig->getcontentBasePath().'podcaster/'.$fileId.'/img'.($slide['slideorder']-1).'.html';
                if (file_exists($html))
                {
                    unlink($html);
                }

                $text = $this->objConfig->getcontentBasePath().'podcaster/'.$fileId.'/text'.($slide['slideorder']-1).'.html';
                if (file_exists($text))
                {
                    unlink($text);
                }
    ...
    {
        $fullPath = $this->objConfig->getcontentBasePath().'podcaster_slide_thumbnails/'.$slideId.'.jpg';
