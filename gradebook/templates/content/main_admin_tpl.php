...
echo '<div class="gradebook-home">';
echo '<h1>'.$esc($courseName).' '.$esc($L('title')).'</h1>';

echo '<div class="gradebook-home-actions">'
    .'<a href="'.$this->uri(array('action'=>'assessmentPlan')).'">'.$esc($L('assessmentplan')).'</a>'
    .' &nbsp; <a href="'.$this->uri(array('action'=>'assessmentSheet')).'">'.$esc($L('assessmentsheet')).'</a>'
    .'</div>';
...
if ($studentCount === 0) {
    ...
    echo '<div style="overflow-x:auto"><table class="table gradebook-summary-table">'
        .'<thead><tr>'
...
if (empty($planRows)) {
    ...
    if ($selectedRow) {
        ...
        echo '<div style="overflow-x:auto"><table class="table gradebook-assessment-results">'
            .'<thead><tr>'
            .'<th>'.$esc($L('studentNumber')).'</th>'
            .'<th>'.$esc($L('student')).'</th>'
            .'<th>'.$esc($L('mark')).'</th>'
