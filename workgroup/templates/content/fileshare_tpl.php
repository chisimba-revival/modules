<?php
    ...
    $pageTitle->str=
		ucwords(
			$this->objLanguage->code2Txt("mod_workgroup_heading",'workgroup')) 
			. " : " . $contextTitle
	...
	$objTable->addHeaderCell($objLanguage->languageText('mod_workgroup_filename','workgroup'));
	//$objTable->addHeaderCell($objLanguage->languageText('mod_fileshare_size','fileshare'));
	$objTable->addHeaderCell($objLanguage->languageText('mod_workgroup_title','workgroup'));
	$objTable->addHeaderCell($objLanguage->languageText('mod_workgroup_description','workgroup'));
	$objTable->addHeaderCell($objLanguage->languageText('mod_workgroup_version','workgroup'));
	...
	if (empty($files)) {
		$objTable->startRow();
		$objTable->addCell("<div align=\"center\" class=\"noRecordsMessage\">".$objLanguage->languageText('mod_workgroup_norecords','workgroup')."</div>",null,null,null,null,'colspan="7"');
		$objTable->endRow();
	}
	else {
		$rowClass = 'odd';
        foreach ($files as $file)
        {
	        ...
	        $objConfirm->setConfirm(
	            $iconDelete->show(),
				$uri,
				$objLanguage->languageText('mod_workgroup_suredelete','workgroup')
			);
    ...
    echo("<a href=\"".$uri."\">".$this->objLanguage->code2Txt('mod_workgroup_return','workgroup')."</a>");                                        
?>
