...
class mailqueuehealth extends dbTable
{
 ...
 public function summary(){
  ...
  try{$counts=$this->getArray("SELECT status,COUNT(*) AS total FROM tbl_communications_outbox WHERE channel='email' GROUP BY status");foreach((array)$counts as $count)$row[$count['status']]=(int)$count['total'];$last=$this->getArray("SELECT MAX(sent_at) AS last_sent FROM tbl_communications_outbox WHERE channel='email' AND status='sent'");if(isset($last[0]['last_sent']))$row['last_sent']=$last[0]['last_sent'];$heartbeat=$this->getRow('id','mail','tbl_communications_worker_state');if(is_array($heartbeat)&&!empty($heartbeat['last_run_at'])){$row['last_run']=$heartbeat['last_run_at'];$age=time()-strtotime($heartbeat['last_run_at']);$row['timer']=$age<=180?'running':'stalled';}}
