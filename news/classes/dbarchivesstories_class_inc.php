...
*/
class dbarchivesstories extends dbtable
{
    ...
    */
    ...
    {
        parent::init('tbl_archives_stories');

        $this->objArchKeywords = $this->getObject('dbarchiveskeywords');
    ...
    */
    public function addStory($storyTitle, $storyDate, $storyCategory, $storyLocation, $storyText, $storySource, $storyImage, $tags, $keyTags, $publishDate = NULL, $sticky='N', $status)
    {
        ...
        if ($storyId != FALSE) {
            $this->objArchKeywords->addStoryKeywords($storyId, $keyTags);

            $objTags = $this->getObject('dbarchivestags');
    ...
    public function getTopStoriesFormatted()
    {
        ...
        if ((is_countable($stories) ? count($stories) : 0) == 0) {
            ...
            foreach ($stories as $story)
            {
                ...
                $output .= $objWashout->parseText($objTrimString->strTrim(strip_tags($story['storytext']), $trimsize, TRUE));
...
WHERE tbl_news_stories.storycategory = \''.$category.'\' ';
    ...
    */
    ...
    {
        $sql = 'SELECT tbl_archives_stories.*, categoryname, name as location FROM tbl_archives_stories
INNER JOIN tbl_archives_categories ON (tbl_archives_stories.storycategory=tbl_archives_categories.id)
LEFT JOIN tbl_geonames ON (tbl_archives_stories.storylocation=tbl_geonames.geonameid)
ORDER BY storydate DESC';
    ...
    *
    */
    public function getArchivedStories()
    {
        $sql = 'select * from tbl_archives_stories order by status';
...
*/
    ...
    */
    ...
    {
        $sql = 'SELECT tbl_news_stories.*, tbl_news_stories.id as storyid, latitude, longitude, name as location FROM tbl_news_stories INNER JOIN tbl_geonames ON (tbl_news_stories.storylocation = tbl_geonames.geonameid) WHERE dateavailable <= \''.strftime('%Y-%m-%d %H:%M:%S', time()).'\'';
    ...
    public function getMonthStories($category, $month=NULL, $year=NULL)
    {
        ...
        INNER JOIN tbl_news_categories ON (tbl_news_stories.storycategory=tbl_news_categories.id)
        LEFT JOIN tbl_geonames ON (tbl_news_stories.storylocation=tbl_geonames.geonameid)
        WHERE tbl_news_stories.storycategory = \''.$category.'\'  AND dateavailable <= \''.strftime('%Y-%m-%d %H:%M:%S', time()).'\' 
    ...
    */
    public function getFirstStory($category, $order)
    {
        ...
        $sql = 'SELECT tbl_news_stories.*, tbl_news_stories.id as storyid, latitude, longitude FROM tbl_news_stories LEFT JOIN tbl_geonames ON (tbl_news_stories.storylocation = tbl_geonames.geonameid) WHERE storycategory=\''.$category.'\' AND dateavailable <= \''.strftime('%Y-%m-%d %H:%M:%S', time()).'\' ORDER BY '.$order.' LIMIT 1';
    ...
    */
    public function getCategoryTitles($category, $order)
    {
        ...
        $sql = 'SELECT tbl_news_stories.id as storyid, storytitle FROM tbl_news_stories WHERE storycategory=\''.$category.'\' AND dateavailable <= \''.strftime('%Y-%m-%d %H:%M:%S', time()).'\' ORDER BY '.$order;
