...
class certificate_service extends controller
{
    ...
    public function dispatch($action)
    {
        ...
        switch((string)$action){
            case 'createbase': return $this->createBase();
            case 'createsigner': return $this->createSigner();
    ...
    private function createBase(){if(!$this->user->isAdmin()||!$this->validPost()){return $this->manage($this->context->getContextCode());}$result=$this->service->createBase(array('name'=>$this->param('name'),'organisation'=>$this->param('organisation'),'companyName'=>$this->param('company_name'),'companyLocation'=>$this->param('company_location'),'websiteUrl'=>$this->param('website_url'),'primaryColour'=>$this->param('primary_colour'),'accentColour'=>$this->param('accent_colour')),$this->user->userId());if(!empty($result['ok'])&&!empty($_FILES['logo']['name'])){$result['ok']=$this->service->storeImageAsset($_FILES['logo'],'logo',$result['id']);}return $this->nextAction(null,array($result['ok']?'message':'error'=>$result['ok']?'saved':'invalid'));}
    private function createSigner(){if(!$this->user->isAdmin()||!$this->validPost()){return $this->manage($this->context->getContextCode());}$result=$this->service->createSigner(array('name'=>$this->param('name'),'title'=>$this->param('title')),$this->user->userId());if(!empty($result['ok'])&&!empty($_FILES['signature']['name'])){$result['ok']=$this->service->storeImageAsset($_FILES['signature'],'signature',$result['id']);}return $this->nextAction(null,array($result['ok']?'message':'error'=>$result['ok']?'saved':'invalid'));}
    private function assignCourse($contextCode){if(!$this->mayManageCourse($contextCode)||!$this->validPost()){return $this->manage($contextCode);}$result=$this->service->assign('course',$contextCode,$this->param('base_id'),$this->param('signer_id'),$this->user->userId());return $this->nextAction(null,array($result['ok']?'message':'error'=>$result['ok']?'assigned':'invalid'));}
