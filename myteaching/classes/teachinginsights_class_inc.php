...
{
 private $user;private $userContext;private $contexts;private $modules;private $language;private $presence;private $time;
 public function init(){
  ...
  $this->presence=$this->getObject('loggedinusers','security');
 ...
 public function courseInsights($userId){
  ...
  foreach(array_values(array_unique((array)$this->userContext->getContextWhereLecturer($userId))) as $code){
   ...
   $activity=$this->studentActivity($code,$students);
   $rows[]=array('code'=>(string)$code,'title'=>(string)$details['title'],'students'=>count($students),'online'=>count(array_filter($activity,fn($person)=>$person['online'])),'student_activity'=>$activity,'progress'=>$this->averageProgress($code,$students),'outstanding'=>$this->outstanding($code,$students));
  }
  usort($rows,fn($a,$b)=>strcasecmp($a['title'],$b['title']));
  return $rows;
 }
 private function memberId($member){return (string)($member['userid']??$member['userId']??$member['id']??'');}
 private function studentActivity($code,array $students){
  $online=array();foreach((array)$this->presence->getActiveUsersInContext($code) as $person)$online[(string)$person['userid']]=$person;
  $rows=array();foreach($students as $student){$id=$this->memberId($student);if($id==='')continue;$isOnline=isset($online[$id]);$last=$isOnline?(string)$online[$id]['whenlastactive']:(string)$this->user->getLastLoginDate($id);$rows[]=array('id'=>$id,'name'=>trim((string)($student['firstname']??'').' '.(string)($student['surname']??'')),'online'=>$isOnline,'lastlogin'=>$last===''?'':$this->time->formatDateTime($last));}
  usort($rows,fn($a,$b)=>($a['online']===$b['online'])?strcasecmp($a['name'],$b['name']):($a['online']?-1:1));return $rows;
 }
 ...
 public function renderCard(array $course){
  ...
  $progress=$course['progress']===null?$this->text('not_available','Not available'):$course['progress'].'%';$html='<div class="teaching-insight-course"><dl><div><dt>'.$e($this->text('students','[-readonlys-]',array())).'</dt><dd>'.$course['students'].'</dd></div><div class="presence-metric presence-metric--online"><dt>'.$e($this->text('online_now','Online now')).'</dt><dd>'.$course['online'].'</dd></div><div><dt>'.$e($this->text('average_progress','Average progress')).'</dt><dd>'.$e($progress).'</dd></div><div><dt>'.$e($this->text('outstanding','Assessments outstanding')).'</dt><dd>'.array_sum(array_column($course['outstanding'],'count')).'</dd></div></dl><div class="teaching-insight-course__queue"><label for="assessment-'.$e($course['code']).'">'.$e($this->text('select_assessment','Select work awaiting marking')).'</label><select id="assessment-'.$e($course['code']).'" data-course="'.$e($course['code']).'" onchange="this.nextElementSibling.href=this.options[this.selectedIndex].dataset.url">';
   if(!$course['outstanding'])$html.='<option value="">'.$e($this->text('no_outstanding','Nothing awaiting marking')).'</option>';else{foreach($course['outstanding'] as $item){$reviewWord=$item['count']===1?'review':'reviews';$queueLabel=$this->text('reviews_awaiting','[-COUNT-] [-readonly-] [-REVIEWS-] awaiting action',array('COUNT'=>$item['count'],'REVIEWS'=>$reviewWord));$html.='<option value="'.$e($item['provider'].'|'.$item['activity']).'" data-url="'.$e($item['url']).'">'.$e($item['name'].' — '.$queueLabel).'</option>';}}
  if(!$course['outstanding'])$html.='</select><button type="button" class="button" disabled>'.$e($this->text('action_later','Open action')).'</button></div>';
  else $html.='</select><a class="button" href="'.$e($course['outstanding'][0]['url']).'">'.$e($this->text('action_later','Open action')).'</a></div>';
  $html.='<details class="teaching-presence"><summary>'.$e($this->text('student_activity','View [-readonly-] activity',array())).'</summary><div class="teaching-presence__list">';
  if(!$course['student_activity'])$html.='<p>'.$e($this->text('no_students','No [-readonlys-] are enrolled.',array())).'</p>';
  foreach($course['student_activity'] as $person){$last=$person['lastlogin']!==''?$person['lastlogin']:$this->text('no_login_recorded','No login recorded');$state=$person['online']?$this->text('active_now','Active now'):$this->text('last_login','Last login').': '.$last;$html.='<div class="teaching-presence__person"><span class="presence-dot'.($person['online']?' presence-dot--online':'').'" aria-hidden="true"></span><strong>'.$e($person['name']).'</strong><span>'.$e($state).'</span></div>';}
