...
/**
 * Model class for the table tbl_mxit_suggested
 ...
 * @see       References to other sections (if any)...
 */
class dbSuggested extends dbTable
{
    /**
    ...
    /**
     * Return a single record in the tbl_mxit_suggested.
     *
     * @param $id is the id taken from the tbl_mxit_suggested
     */
  	...
  	public function insertRecord($word, $definition) 
    {
        ...
        if (empty($word) && empty($definition)) {
            ...
            return $this->insert($arrayOfRecords, 'tbl_mxit_suggested');
        }
    }
    ...
    {
        return $this->delete('id', $id, 'tbl_mxit_suggested');
    }
    ...
    {
        return $this->update('id', $id, $arrayOfRecords, 'tbl_mxit_suggested');
    }
}
?>
