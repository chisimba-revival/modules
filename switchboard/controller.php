 ...
 * Switchboard
 *
 * A swtichboard for enabling switching between different views.
 *
 * PHP version 5
 *
...
*
*/
class switchboard extends controller
{
    ...
    */
    ...
    private function __linkedit()
    {
        ...
        return 'switchboardlinkedit_tpl.php';
    }
    
    /**
     * 
     * Save the results of submitting the linkedit() template
     * from above.
     * 
     * @return string The id of the save or an error code
     * @access private
     * 
     */
    private function __linksave()
    {
        // Initialise the object that will do the saving.
        $objDb = $this->getObject('dbswitchboardlinks', 'switchboard');
        ...
        $iconurl = $this->getParam('iconurl', NULL);
        $link = $this->getParam('link', NULL);
        $title = $this->getParam('title', NULL);
        $description = $this->getParam('description', NULL);
        if ($mode == 'edit') {
            $objDb->saveLink($id, $iconurl, $link, $title, $description);
        } else {
            $id = $objDb->addLink($iconurl, $link, $title, $description);
        }
