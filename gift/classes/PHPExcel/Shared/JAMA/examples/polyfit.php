...
* Pre-Conditions: the system is not underdetermined: (is_countable($X) ? sizeof($X) : 0) > $n+1
*/
function polyfit($X, $Y, $n) {
	for ($i = 0; $i < (is_countable($X) ? sizeof($X) : 0); ++$i)
		for ($j = 0; $j <= $n; ++$j)
			$A[$i][$j] = pow($X[$i], $j);
	for ($i=0; $i < (is_countable($Y) ? sizeof($Y) : 0); ++$i)
		$B[$i] = array($Y[$i]);
