...
class workshoprenderer extends ChisimbaObject
{
    ...
    {
        $e=[self::class,'escape'];
        return '<form '.$this->formAttributes().' method="post" data-workshop-delete data-confirm="'.$e($set['title']."\n\n".$this->text($exam?'exam_delete_notice':'delete_notice')).'" action="'.$e($this->uri(['action'=>$exam?'examdelete':'delete','id'=>$set['id']],'mcqgenerator')).'">'
            ...
            .'<button disabled type="submit" class="button chisimba-button-danger">'.$this->getObject('iconservice','ui')->render('trash-2',['decorative'=>true]).'<span>'.$e($this->text($exam?'exam_delete':'delete_set')).'</span></button></form>';
    }
    ...
    public function form($action,$token,$id=''){return '<form '.$this->formAttributes().' method="post" enctype="multipart/form-data"'.(in_array($action,['generate','generatepart','more'],true)?' data-workshop-generate data-pending="'.self::escape($this->text('generating')).'"':'').' action="'.self::escape($this->uri(['action'=>$action,'id'=>$id],'mcqgenerator')).'"><input type="hidden" name="csrf_token" value="'.self::escape($token).'">';}
}
