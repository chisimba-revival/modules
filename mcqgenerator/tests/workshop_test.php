...
$good=['stem'=>'Which plant has flowers?','options'=>['Tree','Rock','Cloud','Sand'],'correctIndex'=>0,'sourceBasis'=>'The tree has flowers.'];
$candidates=$service->candidates([$good,$diagnosticQuestion,$good],[['code'=>'quote','question'=>2]]);
check($candidates[0]['included']&&!$candidates[1]['included'],'flagged candidates excluded initially');
$review=$service->review($candidates,3);
check(count($service->selected($review))===2,'excluded invalid candidate does not block final set');
$review[1]['included']=true;
rejects(fn()=>$service->review($review,3),'included duplicates must be corrected');
$review[1]['options']=$good['options'];
$review=$service->review($review,3);
check(count($service->selected($review))===3,'author can include checked paraphrase');
foreach($review as &$candidate)$candidate['included']=false;unset($candidate);
rejects(fn()=>$service->selected($review),'empty final set blocked');
$review[2]['included']=true;
$out=$export->text(['title'=>'Selective export','reviewed'=>1,'questions_json'=>json_encode($review)]);
check(str_contains($out,'1. Which plant')&&!str_contains($out,'2. '),'export consecutive numbering');
check(isset($generator->generate($sourceText,3)['candidates']),'rejected response retains review candidates');
echo "PASS selective review, human inclusion, malformed exclusion, empty selection and consecutive export numbering.\n";
