...
$store=$engine->getObject('workshopstore','mcqgenerator');$id=null;
try{
 $q=['stem'=>'Original?','options'=>['A','B','C','D'],'correctIndex'=>1,'sourceBasis'=>'Original evidence.','included'=>false];
 $id=$store->createSet('local-append-fixture','Disposable append test',str_repeat('Source. ',30),[],1);$store->finish($store->one($id),[$q]);$old=$store->one($id);
...
}finally{if($id){$row=$store->one($id);$store->removeOwned($id,$row['ownerid'],(int)$row['version']);}}
