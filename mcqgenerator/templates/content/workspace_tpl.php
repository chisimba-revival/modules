...
if(!$workshopRow){
 ...
 echo '</section><section class="chisimba-form-card chisimba-form-card--wide"><h2>'.$e($set['title']).'</h2><details><summary>'.$t('saved_source').'</summary><p class="chisimba-preserve-whitespace">'.nl2br($e($set['source_text'])).'</p></details></section>';
 ...
 if(!$questions){
  ...
  echo '<section class="chisimba-form-card chisimba-form-card--wide"><h2>'.$t('review_heading').'</h2><p>'.$t('review_notice').'</p><div class="chisimba-form-field"><label for="set-title">'.$t('set_title').'</label><input id="set-title" name="title" required maxlength="200" value="'.$e($workshopTitle?:$set['title']).'"></div></section>';
