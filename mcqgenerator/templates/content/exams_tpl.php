...
if(!$examRow){
 ...
 if($examChapter){
  ...
  foreach($source as $i=>$q){
   ...
   echo '<p>'.$t(($q['type']??'mcq')==='short_answer'?'type_short_answer':'type_mcq').'</p><details><summary>'.$t('exam_preview').'</summary>'.$r->answerPreview($q).'<p>'.$e($q['sourceBasis']).'</p></details></article>';
  }
 ...
 foreach(array_slice($examSets,0,20) as $set){echo '<a class="button chisimba-button-secondary" href="'.$e($this->uri(['action'=>'examview','id'=>$examRow['id'],'setid'=>$set['id'],'page'=>$examPage],'mcqgenerator')).'#chapters">'.$this->getObject('iconservice','ui')->render('book-open',['decorative'=>true]).'<span>'.$e($set['title']).' · '.$t(($set['question_type']??'mcq')==='short_answer'?'type_short_answer':'type_mcq').'</span>'.(isset($chapterCounts[$set['id']])?'<span class="chisimba-pill chisimba-pill--success" aria-label="'.$e($chapterCounts[$set['id']].' '.$r->text('exam_in_exam')).'">'.$chapterCounts[$set['id']].'</span>':'').'</a>';}
 ...
 foreach($content['questions'] as $i=>$entry){
  ...
  echo '<fieldset data-exam-entry class="'.$card.' chisimba-form-card--compact"><legend>'.($i+1).'. '.$e($entry['chapter']).'</legend><p><strong>'.$e($q['stem']).'</strong></p><p>'.$t(($q['type']??'mcq')==='short_answer'?'type_short_answer':'type_mcq').'</p><details><summary>'.$t('exam_preview').'</summary>'.$r->answerPreview($q).'</details><div class="chisimba-control-row">';
