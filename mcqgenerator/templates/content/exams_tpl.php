...
if(!$examRow){
 ...
 foreach($content['questions'] as $i=>$entry){
  ...
  echo '<fieldset class="'.$card.'"><legend>'.($i+1).'. '.$e($entry['chapter']).'</legend><p><strong>'.$e($q['stem']).'</strong></p><details><summary>'.$t('exam_preview').'</summary><ol type="A">';foreach($q['options'] as $option)echo '<li>'.$e($option).'</li>';echo '</ol><p>'.$t('correct').': '.$e(chr(65+$q['correctIndex']).'. '.$q['options'][$q['correctIndex']]).'</p></details><div class="chisimba-form-actions" style="align-items:end;flex-wrap:wrap">';
  echo '<label><input type="checkbox" name="exam[entries]['.$key.'][keep]" value="1"'.($input===null||($input['keep']??'')==='1'?' checked':'').'> '.$t('exam_keep').'</label>';
