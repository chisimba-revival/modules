...
class ADORecordSet_array_sybase extends ADORecordSet_array {
	...
	global $ADODB_sybase_mths;
		...
		//Dec 30 2000 12:00AM
		if (!preg_match("~([A-Za-z]{3})[-/\. ]+([0-9]{1,2})[-/\. ]+([0-9]{4})~",$v, $rr)) return parent::UnixDate($v);
	...
	global $ADODB_sybase_mths;
		...
		if (!preg_match("~([A-Za-z]{3})[-/\. ]([0-9 ]{1,2})[-/\. ]([0-9]{4}) +([0-9]{1,2}):([0-9]{1,2}) *([apAP]{0,1})~",$v, $rr)) return parent::UnixTimeStamp($v);
		if ($rr[3] <= TIMESTAMP_FIRST_YEAR) return 0;
