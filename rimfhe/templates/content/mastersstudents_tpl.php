...
if($mode =='edit'){
    $editMode= 'update';
    $pageHeading->str = $this->objLanguage->languageText('mod_rimfhe_pgheadingeditmasters', 'rimfhe');
}
else{
    $pageHeading->str = $this->objLanguage->languageText('mod_rimfhe_pgheadingmasters', 'rimfhe');
    $editMode= '';
}
...
$doctorateStudents = new form ('mastersstudents', $this->uri(array('action'=>'mastersstudents' , 'editmode' => $editMode), 'rimfhe'));
...
if($mode == 'edit'){
    $txtId = new textinput("mastersid", $arrEdit['id'], 'hidden');
    $table->startRow();
