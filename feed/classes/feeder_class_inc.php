...
class feeder extends ChisimbaObject
{
     ...
     */
    public function output($format = "RSS2.0")
    { //RSS0.91, RSS1.0, RSS2.0, PIE0.1, MBOX, OPML, ATOM0.3, HTML, JS
        switch ($format)
        {
            ...
            case "RSS0.91":
                return $this->objFeeder->saveFeed("RSS0.91", "feed.xml");
                break;

            case "RSS1.0":
                return $this->objFeeder->saveFeed("RSS1.0", "feed.xml");
                break;

            case "RSS2.0":
                return $this->objFeeder->saveFeed("RSS2.0", "feed.xml");
                break;

            case "PIE0.1":
                return $this->objFeeder->saveFeed("PIE0.1", "feed.xml");
                break;

            case "MBOX":
                return $this->objFeeder->saveFeed("MBOX", "feed.xml");
                break;

            case "OPML":
                return $this->objFeeder->saveFeed("OPML", "feed.xml");
                break;

            case "ATOM0.3":
                return $this->objFeeder->saveFeed("ATOM0.3", "feed.xml");
                break;

            case "HTML":
                return $this->objFeeder->saveFeed("HTML", "feed.xml");
                break;

            case "JS":
                return $this->objFeeder->saveFeed("JS", "feed.xml");
                break;
        } //end switch
    } //end function
} //end class
