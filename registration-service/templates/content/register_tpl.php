...
<?php if(!empty($registrationPurchase['code'])): ?><input type="hidden" name="selected_product" value="<?php echo $e($registrationPurchase['code']); ?>"><?php endif; ?>
...
<div class="chisimba-form-field"><label for="registration-identity-type"><?php echo $e($t('identity_document_type')); ?></label><select id="registration-identity-type" name="identity_document_type" required><option value=""></option><?php foreach(array('south_african_id','passport','national_identity_document') as $identityType): ?><option value="<?php echo $e($identityType); ?>"<?php echo ($v['identityDocumentType']??'')===$identityType?' selected':''; ?>><?php echo $e($t('identity_type_'.$identityType)); ?></option><?php endforeach; ?></select></div>
<div class="chisimba-form-field"><label for="registration-identity-number"><?php echo $e($t('identity_document_number')); ?></label><input id="registration-identity-number" name="identity_document_number" type="text" maxlength="128" autocomplete="off" aria-describedby="registration-identity-help" value="<?php echo $e($v['identityDocumentNumber'] ?? ''); ?>" required><small id="registration-identity-help" class="chisimba-field-help"><?php echo $e($t('identity_document_help')); ?></small></div>
