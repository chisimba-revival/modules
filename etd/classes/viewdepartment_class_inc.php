<?php
/**
* viewDepartment class extends dbtable
* @package etd
* @filesource
*/
...
/**
* viewDepartment class provides functionality for browsing titles in a department via the viewbrowse class.
*
* @author Megan Watson
...
class viewDepartment extends dbthesis
{
    /**
    ...
    */
    public function listDepartment()
    {
        ...
        $head = $this->objLanguage->languageText('word_departments');
        $lbNoDepartment = $this->objLanguage->languageText('mod_etd_nodepartmentsavailable', 'etd');
        ...
        if(!empty($data)){
            ...
            foreach($data as $item){
                ...
                $objLink = new link($this->uri(array('action' => 'viewdepartment', 'id' => $item['id'])));
            ...
            $str = '<p class="noRecordsMessage">'.$lbNoDepartment.'</p>';
        }
    ...
    */
    public function getDepartments($limit = 10, $start = NULL, $joinId = NULL)
    {
        ...
        $sql .= "WHERE submit.id = thesis.submitid AND dc.id = thesis.dcmetaid AND thesis.thesis_degree_discipline != '' ";
    ...
    */
    public function getData($limit = 10, $start = NULL, $joinId = NULL)
    {
        ...
        $sql .= "AND submit.submissiontype = '{$this->subType}' AND submit.status = 'archived' ";
        $sql .= "AND thesis.thesis_degree_discipline = '{$department}' ";
