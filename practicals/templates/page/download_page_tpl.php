...
if ((is_countable($data) ? count($data) : 0)==0){ // if the file has been deleted
    ...
    $list=$this->objFile->getArray("select id from tbl_practicals_blob where fileId='$fileId2' order by segment");
    ...
    {
        $id=$line['id'];
        $filedata=$this->objFile->getArray("select * from tbl_practicals_blob where id='$id'");
