...
class dbdiscussionaimarkingjobs extends dbTable
{
    ...
    public function latestStatus($discussionId,$studentId,$context){$sql="SELECT status,error_code,date_updated FROM tbl_discussion_ai_marking_jobs WHERE discussion_id='".$this->e($discussionId)."' AND student_id='".$this->e($studentId)."' AND contextcode='".$this->e($context)."' ORDER BY date_created DESC LIMIT 1";$rows=$this->getArray($sql);return $rows[0]??false;}
