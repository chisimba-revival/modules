...
class dbdiscussionaimarkingjobs extends dbTable
{
    ...
    {
        $active=$this->getArray("SELECT id FROM tbl_discussion_ai_marking_jobs WHERE discussion_id='".$this->e($discussionId)."' AND student_id='".$this->e($studentId)."' AND contextcode='".$this->e($context)."' AND status IN ('queued','running') LIMIT 1");if(!empty($active[0]['id']))return $active[0]['id'];
        ...
        if(!$evidence||count($evidence)>500){return false;}
        $inserted=$this->insert(array('id'=>$id,'contextcode'=>(string)$context,'discussion_id'=>(string)$discussionId,'student_id'=>(string)$studentId,'requester_id'=>(string)$requesterId,'status'=>'queued','rubric_version'=>'discussion-participation-v1','evidence_json'=>json_encode($evidence),'result_json'=>'{}','date_created'=>$now,'date_updated'=>$now));return $inserted===false?false:$id;
    }
    ...
    public function latest($discussionId,$studentId,$context,$requesterId,$admin=false,$evidence=null)
    {
        $sql="SELECT id FROM tbl_discussion_ai_marking_jobs WHERE discussion_id='".$this->e($discussionId)."' AND student_id='".$this->e($studentId)."' AND contextcode='".$this->e($context)."' AND status='completed'".($admin?'':" AND requester_id='".$this->e($requesterId)."'")." ORDER BY date_completed DESC,date_created DESC,id DESC LIMIT 1";
        $rows=$this->getArray($sql);
        $job=empty($rows[0]['id'])?false:$this->getScoped($rows[0]['id'],$context,$requesterId,$admin);
        if($evidence===null){$evidence=$this->getObject('dbpost','discussion')->getAssessmentEvidence($discussionId,$studentId);}
        return $this->getObject('discussionassessmentstate','discussion')->jobMatches($job,$evidence)?$job:false;
    }
    public function latestStatus($discussionId,$studentId,$context)
    {
        // An active older snapshot must finish before another job can be queued.
        $sql="SELECT status,error_code,date_updated FROM tbl_discussion_ai_marking_jobs WHERE discussion_id='".$this->e($discussionId)."' AND student_id='".$this->e($studentId)."' AND contextcode='".$this->e($context)."' ORDER BY (status IN ('queued','running')) DESC,date_created DESC,id DESC LIMIT 1";
        $rows=$this->getArray($sql);$row=$rows[0]??false;
        // Completed state is supplied only by latest(), after comparing its snapshot.
        return is_array($row)&&$row['status']==='completed'?false:$row;
    }
