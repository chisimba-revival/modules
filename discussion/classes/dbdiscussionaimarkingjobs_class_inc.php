...
class dbdiscussionaimarkingjobs extends dbTable
{
    ...
    public function enqueue($context,$discussionId,$studentId,$requesterId)
    {
        ...
        $this->insert(array('id'=>$id,'contextcode'=>(string)$context,'discussion_id'=>(string)$discussionId,'student_id'=>(string)$studentId,'requester_id'=>(string)$requesterId,'status'=>'queued','rubric_version'=>'discussion-participation-v1','evidence_json'=>json_encode($evidence),'result_json'=>'{}','date_created'=>$now,'date_updated'=>$now));return $id;
    }
