<?php
/**
 * Subscriptions twitterizer dbtable derived class
 *
...
class dbsubs extends dbTable {
    ...
    public function init($tableName = null, $pearDb = null, $errorCallback = 'globalPearErrorHandler') {
        parent::init ( 'tbl_twitterizer_subs' );
    }

    /**
     * Private method to insert a record to the twitterizer subs table.
     *
     * This method will add a simple record to the table
     *
     * @param array $jid - string Jabber ID
     * @return string $id
     */
    public function addRecord($jid) {
        $insarr = array('jid' => $jid, 'status' => 1, 'datesent' => $this->now());
        if($this->userExists($jid) === FALSE) {
            return $this->insert ( $insarr, 'tbl_twitterizer_subs' );
        }
        else {
            if($this->userExists($jid) === TRUE) {
                ...
                return $this->update ( 'id', $id, $insarr, 'tbl_twitterizer_subs' );
            }
            else {
                return FALSE;
            }
    ...
    public function inactiveRecord($jid) {
        ...
        if($this->userExists($jid) === TRUE) {
            ...
            return $this->update ( 'id', $id, $actarr, 'tbl_twitterizer_subs' );
        }
        else {
            return FALSE;
        }
