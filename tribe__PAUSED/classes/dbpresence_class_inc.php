<?php
/**
 * Presence dbtable derived class
 *
 * Class to interact with the database for the tribe presence
 *
 * PHP version 5
 *
 ...
 */
class dbpresence extends dbTable {
    /**
     * Constructor
     *
     */
    ...
    public function updatePresence($userarr) {
        ...
        if ($status === FALSE) {
            $this->insert ( $insarr, 'tbl_tribe_presence' );
        } else {
            // update the presence info for this user
            $this->update ( 'id', $status [0] ['id'], $insarr, 'tbl_tribe_presence' );
        }
    }
    ...
    private function addRecord($insarr) {
        ...
        return $this->insert ( $insarr, 'tbl_tribe_presence' );
    }
