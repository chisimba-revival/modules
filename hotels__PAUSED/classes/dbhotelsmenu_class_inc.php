...
class dbhotelsmenu extends dbtable
{
    ...
    public function toolbar($current='storyview')
    {
        ...
        $toolbar = $objSysConfig->getValue('SHOWTOOLBAR', 'hotels');

        if ($toolbar != 1) {
            return '';
        } else {
            //$options = array('storyview', 'showmap', 'map', 'viewtimeline', 'timeline', 'themecloud');
			$options = array('storyview', 'showmap', 'map');
			...
			$menuOptions = array('storyview', 'showmap');
            ...
            foreach ($menuOptions as $option)
            {
                ...
                $link->link = $this->objLanguage->languageText('mod_hotels_toolbar_'.$option, 'hotels');
    ...
    {
        $sql = 'SELECT id FROM tbl_hotels_menu WHERE itemtype=\'category\' and itemvalue=\''.$id.'\'';
