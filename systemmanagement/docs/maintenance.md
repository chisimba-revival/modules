...
The audience, subject and message remain on the page after a handled submission error. No email queued means that submission queued nothing. A partial or uncertain result may have queued some messages: inspect the delivery history before trying again. The draft also remains after success for reference; pressing Queue email notice again creates a new send. With JavaScript enabled, these controls update in place: feedback appears beside the action, both columns keep their text and position, and delivery history refreshes after a response. Saving the plan or changing availability retains the email fields. A failed connection leaves your text in place and does not retry the action automatically; check delivery history before retrying an email. Select Save draft before leaving the page; it stores unfinished text for this signed-in session without queueing email. Drafts are separate for each administrator and expire with the session. Keep a separate copy if you need it after signing out. The status beside the form distinguishes draft text from text already queued; delivery history remains the source for delivery results.
