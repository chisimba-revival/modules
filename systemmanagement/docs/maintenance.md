...
The audience, subject and message remain on the page after a handled submission error. No email queued means that submission queued nothing. A partial or uncertain result may have queued some messages: inspect the delivery history before trying again. The draft also remains after success for reference; pressing Queue email notice again creates a new send. Saving the plan or changing availability retains the email fields. Select Save draft before leaving the page; it stores unfinished text for this signed-in session without queueing email. Drafts are separate for each administrator and expire with the session. Keep a separate copy if you need it after signing out. The status beside the form distinguishes draft text from text already queued; delivery history remains the source for delivery results.
