...
class systemmanagement extends controller
{
    ...
    public function dispatch($action){$action=(string)$action;if($action==='offline')return $this->offline();if(!$this->user->isAdmin())return 'noaccess_tpl.php';if($action==='save')return $this->save();if($action==='setmaintenance')return $this->setMaintenance();if($action==='deletenotice')return $this->deleteNotice();return $this->index();}
    ...
    private function setMaintenance(){if(!$this->validPost())return $this->index('','Your session expired. Please try again.');$offline=$this->param('state')==='offline';$this->config->changeParam('MAINTENANCE_ENABLED','systemmanagement',$offline?'1':'0');return $this->index($offline?'The site is now offline for non-administrators. Administrator access remains available.':'The site is now online.');}
