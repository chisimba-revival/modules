...
$currentLocal=$this->objTimeAndDate->inTimezone($this->objTimeAndDate->nowStorage());
$currentYear=(int)$currentLocal->format('Y');
...
<fieldset class="chisimba-form-field"><legend>Closing date and time</legend><div class="chisimba-date-time-fields"><label for="essay-closing-day">Day</label><select id="essay-closing-day" name="closing_day"><?php for($day=1;$day<=31;$day++): ?><option value="<?php echo $day; ?>"<?php echo $day===$closingDay?' selected':''; ?>><?php echo $day; ?></option><?php endfor; ?></select><label for="essay-closing-month">Month</label><select id="essay-closing-month" name="closing_month"><?php for($month=1;$month<=12;$month++): $monthName=(new DateTimeImmutable('2000-01-01'))->setDate(2000,$month,1)->format('F'); ?><option value="<?php echo $month; ?>"<?php echo $month===$closingMonth?' selected':''; ?>><?php echo $e($monthName); ?></option><?php endfor; ?></select><label for="essay-closing-year">Year</label><select id="essay-closing-year" name="closing_year"><?php for($year=$currentYear;$year<=$currentYear+10;$year++): ?><option value="<?php echo $year; ?>"<?php echo $year===$closingYear?' selected':''; ?>><?php echo $year; ?></option><?php endfor; ?></select><label for="essay-closing-time">Time (24-hour)</label><input id="essay-closing-time" type="time" name="closing_time" value="<?php echo $e($closingTime); ?>" required></div></fieldset>
