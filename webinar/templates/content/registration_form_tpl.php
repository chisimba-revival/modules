...
echo '<form method="post" action="'.$e($this->uri($action==='register'?['action'=>'view','id'=>$record['id']]:['action'=>$action],'webinar')).($action==='register'?'#booking':'').'" class="chisimba-flow"'.($action==='register'?' data-booking-form data-check-url="'.$e($this->uri(['action'=>'booking_status'],'webinar')).'"':'').'>'
    .'<input type="hidden" name="csrf_token" value="'.$e($this->getVar('webinarCsrf')).'">';
if($action==='register'){
 echo '<input type="hidden" name="check_token" value="'.$e($this->getVar('webinarCheckToken')).'"><p role="status" aria-live="polite" data-booking-status hidden></p>';
 ...
 echo '<div class="chisimba-form-field"><label><input type="checkbox" name="consent" value="1" required '.($input['consent']==='1'?'checked':'').'> '.$e($r->text('consent')).'</label></div><p>'.$e($r->text($this->getObject('webinarregistrationservice','webinar')->requiresEmailVerification()?'registration_help':'registration_help_immediate')).'</p>';
}else{
...
if($action==='register')echo '<script defer src="'.$e($this->getResourceUri('booking.js','webinar')).'?v=0.018"></script>';
