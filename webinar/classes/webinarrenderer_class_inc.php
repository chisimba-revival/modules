...
class webinarrenderer extends ChisimbaObject
{
 ...
 public function date($r){$d=$this->data($r);if(empty($r['presented_at']))return '';try{$date=new DateTimeImmutable($r['presented_at'],new DateTimeZone($d['timezone']));}catch(Throwable $e){return ''; }return '<p><small>'.self::escape($this->text('presented').' '.$date->format('j F Y')).'</small></p>';}
 public function cards($rows){$html='<div class="chisimba-publication-card-grid">';foreach($rows as $r){$d=$this->data($r);$html.='<article class="chisimba-publication-card"><div class="chisimba-publication-card__media">'.$this->image($d['image']??'',$r['title']).'</div><div class="chisimba-publication-card__body"><h2><a href="'.self::escape($this->uri(['action'=>'view','id'=>$r['id']],'webinar')).'">'.self::escape($r['title']).'</a></h2>'.$this->date($r).'</div></article>';}return $html.'</div>';}
 public function detail($r){$d=$this->data($r);$html='<article class="chisimba-form-section">'.$this->image($d['banner']??$d['image']??'',$r['title']).'<h1>'.self::escape($r['title']).'</h1>'.$this->date($r);
  ...
  if($r['kind']==='webinar'){
   $url=$d['recording']??'';
   if($url&&$this->getObject('contentmediaservice','contentblocks')->videoEmbed($url))$html.='<p><a class="button chisimba-button-primary" href="'.self::escape($url).'">'.$this->getObject('iconservice','ui')->render('play',['decorative'=>true]).'<span>'.self::escape($this->text('watch')).'</span></a></p>';
   else $html.='<p>'.self::escape($this->text('missing')).'</p>';
