...
$this->objequipmentHeading = $this->newObject('htmlheading','htmlelements');
$this->objequipmentHeading->type = 2;
$this->objequipmentHeading->str=$objLanguage->languageText('mod_onlineinvoice_equipmentexpenses','onlineinvoice');
/*********************************************************************************************************************************************************************/

$exchangeRate  = '<a href=http://www.oanda.com/convert/classic/>www.oanda.com/convert/classic</a>';
$equipmentHint  = $this->objLanguage->languageText('mod_onlineinvoice_pleasemouseover','onlineinvoice');
$equipmentExchangeRate = $this->objLanguage->languageText('mod_onlineinvoice_anyexchangerate','onlineinvoice');
$equipmentSuggestedExRate = $this->objLanguage->languageText('mod_onlineinvoice_suggestedexchangerate','onlineinvoice')  . ' ' . $exchangeRate;
$equipmentExpenditure = $this->objLanguage->languageText('mod_onlineinvoice_equipmentexpenditures','onlineinvoice');
...
$descriptionequipment = $this->objLanguage->LanguageText('mod_onlineinvoice_descriptionofequipment', 'onlineinvoice');
 ...
 *help information
 */
$equipmentinstruc  = $this->objLanguage->languageText('mod_onlineinvoice_equipmentinstruc','onlineinvoice');  
$equipmentinformation  = $this->objLanguage->languageText('mod_onlineinvoice_equipmentinformation','onlineinvoice');
$equipment  = $this->objLanguage->languageText('mod_onlineinvoice_equipment','onlineinvoice');
$equipmentexample  = $this->objLanguage->languageText('mod_onlineinvoice_equipmentexample','onlineinvoice');
...
$equipmentreceiptinfo  = $this->objLanguage->languageText('mod_onlineinvoice_equipmentreceiptinfo','onlineinvoice');
...
$helpstring = $equipmentinstruc . '<br />' .$equipmentinformation .'<br />' .$equipment . '<br />'  .$equipmentexample .'<br />'  . $verifyexchangerate . '<br />'  . $exp  . '<br />'  .$options.'<br />'.$equipmentreceiptinfo.'<br />'.$receiptexplanation.'<br />'.$receiptexp.'<br />'.$receiptexp.'<br />'.$affidavitinstruc;
...
$initialdate [] = $this->getSession('invoicedata');
 	if(!empty($initialdate)) {
 		foreach($initialdate as $sesEquip) {
 			$datevalue = $sesEquip['begindate'];
 		}
	}


$this->objequipment = $this->newObject('datepicker','htmlelements');
$name = 'txtequipmentdate';
$date = $datevalue;
$format = 'YYYY-MM-DD';
$this->objequipment->setName($name);
$this->objequipment->setDefaultDate($date);
$this->objequipment->setDateFormat($format);
...
$this->objtxtdescriptionequipment = $this->newObject('textinput','htmlelements');
$this->objtxtdescriptionequipment->textinput("txtdescriptionequipment","",'text');
 ...
 *create a table for lodge details  - place all form headings in*/
        ...
        $myTabequipmentheading  = $this->newObject('htmltable','htmlelements');
        $myTabequipmentheading->width='80%';
        $myTabequipmentheading->border='0';
        $myTabequipmentheading->cellspacing = '10';
        $myTabequipmentheading->cellpadding ='10';

        $myTabequipmentheading->startRow();
        $myTabequipmentheading->addCell("<div align=\"left\">"  . $equipmentHint . "</div>");
        $myTabequipmentheading->endRow();
        
        $myTabequipmentheading->startRow();
        $myTabequipmentheading->addCell("<div align=\"left\">"  . $this->objInfoIcon->show() . $ExchangeRate  . "</div>");
        $myTabequipmentheading->endRow();
        
        $myTabequipmentheading->startRow();
        $myTabequipmentheading->addCell("<div align=\"left\">"  . $equipmentSuggestedExRate . "</div>");
        $myTabequipmentheading->endRow();
        
        $myTabequipmentheading->startRow();
        $myTabequipmentheading->addCell("<div align=\"left\">"  . $equipmentExpenditures  . "</div>");
        $myTabequipmentheading->endRow();
/*********************************************************************************************************************************************************************/
/*create a table for lodge details*/

        $myTabequipment  = $this->newObject('htmltable','htmlelements');
        $myTabequipment->width='70%';
        $myTabequipment->border='0';
        $myTabequipment->cellspacing = '5';
        $myTabequipment->cellpadding ='10';

        $myTabequipment->startRow();
        $myTabequipment->addCell($expensesdate);
        $myTabequipment->addCell($this->objequipment->show());
        $myTabequipment->addCell('');
        $myTabequipment->addCell('');
        $myTabequipment->addCell('');
        $myTabequipment->addCell('');
        $myTabequipment->addCell('');
        $myTabequipment->addCell($displayhelp);
        
        $myTabequipment->endRow();

        $myTabequipment->startRow();
        $myTabequipment->addCell($vendor);
        $myTabequipment->addCell($this->objtxtvendor->show());
        $myTabequipment->endRow();
        
        $myTabequipment->startRow();
        $myTabequipment->addCell($descriptionequipment);
        $myTabequipment->addCell($this->objtxtdescriptionequipment->show());
        $myTabequipment->endRow();

        $myTabequipment->startRow();
        $myTabequipment->addCell($currency);
        $myTabequipment->addCell($this->objcurrencydropdown->show());
        $myTabequipment->endRow();

        $myTabequipment->startRow();
        $myTabequipment->addCell($cost);
        $myTabequipment->addCell($this->objtxtcost->show());
        $myTabequipment->endRow();
        
        $myTabequipment->startRow();
        $myTabequipment->addCell($rate);
        $myTabequipment->addCell($this->objtxtexchange->show());
        $myTabequipment->endRow();
        
        //$myTabequipment->startRow();
        //$myTabequipment->addCell($cost);
        //$myTabequipment->addCell($this->objButtonSubmit->show());
        //$myTabequipment->endRow();
...
/*create tabbox for equipment information*/
$this->loadClass('tabbedbox', 'htmlelements');
$objtabbedbox = new tabbedbox();
$objtabbedbox->addTabLabel('equipment Information');
$objtabbedbox->addBoxContent($myTabequipment->show());
...
$objequipmentForm = new form('equipment',$this->uri(array('action'=>'createequipment')));
$objequipmentForm->displayType = 3;
$objequipmentForm->addToForm($objtabbedbox->show());	
$objequipmentForm->addToForm($objtabboxfile->show() . '<br />' . '<br />' . $objtabreceipt->show() . '<br />'."<div align=\"center\">".$this->objaddlodge->show() . '<br />' . '<br />' . "<div align=\"center\">" . $this->objBack->show() . $this->objnext->show()  . "</div");

//$formlodging = '<form name="lodging" id="lodging" enctype="multipart/form-data" method="post" action="'.$this->formaction.'">';
$objequipmentForm->addRule('txtvendor', 'Please enter vendor name','required');
$objequipmentForm->addrule('txtdescriptionequipment', 'Please enter equipment descriptionequipment', 'reqiured');
$objequipmentForm->addRule('txtcost', 'Please enter cost amount','required');
$objequipmentForm->addRule('txtexchange', 'Please enter the exchange rate','required');

/*********************************************************************************************************************************************************************/
  

//display screen content
echo "<div align=\"center\">" . $this->objequipmentHeading->show()  . "</div>";
echo "<div align=\"right\">" .'<br>'  . $myTabequipmentheading->show() . "</div>";
echo  "<div align=\"left\">"  . $objequipmentForm->show() . "</div";
