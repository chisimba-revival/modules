...
/**create a template for odc expenses the file attachments**/
...
$this->objodcHeading = $this->newObject('htmlheading','htmlelements');
$this->objodcHeading->type = 1;
$this->objodcHeading->str=$objLanguage->languageText('mod_onlineinvoice_odcexpenses','onlineinvoice');
/*********************************************************************************************************************************************************************/

//$exchangeRate  = '<a href=http://www.oanda.com/convert/classic/>www.oanda.com</a>';
//$odcHint  = $this->objLanguage->languageText('mod_onlineinvoice_pleasemouseover','onlineinvoice');
$odcExchangeRate = $this->objLanguage->languageText('mod_onlineinvoice_anyexchangerate','onlineinvoice');
$odcSuggestedExRate = $this->objLanguage->languageText('mod_onlineinvoice_suggestedexchangerate','onlineinvoice')  . ' ' . $exchangeRate;
$odcExpenditures = $this->objLanguage->languageText('mod_onlineinvoice_itemizedexpenditures','onlineinvoice');
...
$descriptionodc = $this->objLanguage->LanguageText('mod_onlineinvoice_descriptionofodc', 'onlineinvoice');
 ...
 *help information
 */
$odcinstruc  = $this->objLanguage->languageText('mod_onlineinvoice_odcinstruc','onlineinvoice');  
$odcinformation  = $this->objLanguage->languageText('mod_onlineinvoice_odcinformation','onlineinvoice');
$odc  = $this->objLanguage->languageText('mod_onlineinvoice_odc','onlineinvoice');
$odcexample  = $this->objLanguage->languageText('mod_onlineinvoice_odcexample','onlineinvoice');
...
$odcreceiptinfo  = $this->objLanguage->languageText('mod_onlineinvoice_odcreceiptinfo','onlineinvoice');
...
$helpstring = $odcinstruc . '<br />' .$odcinformation .'<br />' .$odc . '<br />'  .$odcexample .'<br />'  . $verifyexchangerate . '<br />'  . $exp  . '<br />'  .$options.'<br />'.$odcreceiptinfo.'<br />'.$receiptexplanation.'<br />'.$receiptexp.'<br />'.$receiptexp.'<br />'.$affidavitinstruc;
...
$this->objodcdate = $this->newObject('datepicker','htmlelements');
$name = 'txtodcdate';
$date = $dateitinerary;
$format = 'YYYY-M-DD';
$this->objodcdate->setName($name);
$this->objodcdate->setDefaultDate($date);
$this->objodcdate->setDateFormat($format);
...
$this->objtxtdescriptionodc = $this->newObject('textinput','htmlelements');
$this->objtxtdescriptionodc->textinput("txtodcdesc","",'text');
...
$this->objaddodc  = new button('add', $add);
$this->objaddodc->setToSubmit();
...
/*********************************************************************************************************************************************************************/
/**
 *create a table for odc details  - place all form headings in*/

        $myTabodcheading  = $this->newObject('htmltable','htmlelements');
        $myTabodcheading->width='100%';
        $myTabodcheading->border='0';
        $myTabodcheading->cellspacing = '10';
        $myTabodcheading->cellpadding ='10';

        $myTabodcheading->startRow();
        $myTabodcheading->endRow();
        
        $myTabodcheading->startRow();
        $myTabodcheading->addCell("<div align=\"left\">" ."<div class=\"error\">" . $this->objInfoIcon->show()  . $odcExchangeRate  . "</div>");
        $myTabodcheading->endRow();
        
        $myTabodcheading->startRow();
        $myTabodcheading->addCell("<div align=\"left\">"  .'<b />'. $odcSuggestedExRate . "</div>");
        $myTabodcheading->endRow();
        
        $myTabodcheading->startRow();
        $myTabodcheading->addCell("<div align=\"left\">"  . '<b />'. $odcExpenditures  . "</div>");
        $myTabodcheading->endRow();
/*********************************************************************************************************************************************************************/
/*create a table for odc details*/

        $myTabodc  = $this->newObject('htmltable','htmlelements');
        $myTabodc->width='90%';
        $myTabodc->border='0';
        $myTabodc->cellspacing = '5';
        $myTabodc->cellpadding ='10';

        $myTabodc->startRow();
        $myTabodc->addCell($expensesdate);
        $myTabodc->addCell($this->objodcdate->show());
        $myTabodc->addCell(' ');
        $myTabodc->addCell(' ');
        $myTabodc->addCell(' ');
        $myTabodc->addCell(' ');
        $myTabodc->addCell(' ');
        $myTabodc->addCell(' ');
        $myTabodc->addCell($displayhelp);
        $myTabodc->endRow();


        $myTabodc->startRow();
        $myTabodc->addCell($vendor);
        $myTabodc->addCell($this->objtxtvendor->show());
        $myTabodc->endRow();
        
        $myTabodc->startRow();
        $myTabodc->addCell($descriptionodc);
        $myTabodc->addCell($this->objtxtdescriptionodc->show());
        $myTabodc->endRow();
      

  		  $myTabodc->startRow();
        $myTabodc->addCell($currency);
        $myTabodc->addCell($this->objcurrencydropdown->show());
        $myTabodc->endRow();

        $myTabodc->startRow();
        $myTabodc->addCell($cost);
        $myTabodc->addCell($this->objtxtcost->show());
        $myTabodc->endRow();
        
        
        
        $myTabodc->startRow();
        $myTabodc->addCell($rate);
        $myTabodc->addCell($this->objtxtexchange->show());
        $myTabodc->endRow();
...
/*create tabbox for odc information*/
$this->loadClass('tabbedbox', 'htmlelements');
$objtabbedbox = new tabbedbox();
$objtabbedbox->addTabLabel('odc Information');
$objtabbedbox->addBoxContent($myTabodc->show() . '<br />');

/*create tabbox for attaching odc echange rate file*/
$this->loadClass('tabbedbox', 'htmlelements');
...
$objodcForm = new form('lodging',$this->uri(array('action'=>'submitodcexpenses')));
$objodcForm->displayType = 3;
$objodcForm->addToForm('<br /> '.$objtabbedbox->show()  . '<br />'  .$objtabexchange->show() . '<br />'.$objtabreceipt->show(). '<br />'."<div align=\"right\">".$this->objaddodc->show() ."</div>".'<br />' . "<div align=\"center\">" . $this->objBack->show(). $this->objnext->show() . ' ' ."</div");	
$objodcForm->addRule('txtvendor', 'Please enter vendor name','required');
$objodcForm->addRule('txtcost', 'Please enter cost amount','required');
$objodcForm->addRule('txtcost', 'Please enter a numerical value for cost amount','numeric');
$objodcForm->addRule('txtexchange', 'Please enter exchange rate','required');
...
echo "<div align=\"center\">" . $this->objodcHeading->show()  . "</div>";
echo "<div align=\"right\">" .'<br />'  . $myTabodcheading->show() . "</div>";
echo  "<div align=\"left\">"  . $objodcForm->show() . "</div>";
