...
class kanban extends controller
{
    ...
    private function reorderprojects(){$json=$this->param('response')==='json';if(!$this->validPost())return $json?$this->json(false,'Your session expired.',403):$this->index('','Your session expired.');$board=$this->service->board($this->id('boardid'),'manage');$direction=$this->param('direction');if(!$board||!in_array($direction,array('up','down'),true))return $json?$this->json(false,'Permission denied.',403):$this->forbidden();$rows=array_values(array_filter($this->boards->inScope($board['scopetype'],$board['scopeid'],true),function($row){return $this->auth->allows($row,'manage');}));$position=null;foreach($rows as $index=>$row)if($row['id']===$board['id']){$position=$index;break;}$target=$position===null?null:$position+($direction==='up'?-1:1);if($target!==null&&isset($rows[$target])){$currentOrder=(int)$rows[$position]['sortorder'];$targetOrder=(int)$rows[$target]['sortorder'];if($currentOrder===$targetOrder){foreach($rows as $index=>$row)$this->boards->setSortOrder($row['id'],($index+1)*100);$currentOrder=($position+1)*100;$targetOrder=($target+1)*100;}$this->boards->setSortOrder($board['id'],$targetOrder);$this->boards->setSortOrder($rows[$target]['id'],$currentOrder);}return $json?$this->json(true,'Board order updated.'):$this->index('Board order updated.');}
