...
class kanban extends controller
{
    ...
    private function index($message='',$error=''){
        if($this->param('response')==='json' && ($_SERVER['REQUEST_METHOD']??'')==='POST')return $this->json($error==='', $error!==''?$error:$message, $error!==''?422:200);
        $recovery=array();
        if($error!=='' && ($_SERVER['REQUEST_METHOD']??'')==='POST')foreach(array('title','description','notes','grants') as $field){$value=$this->param($field);if($value!=='')$recovery[$field]=$value;}
        $this->setVar('kanbanRecovery',$recovery);
    ...
    private function requestedScope(){
        ...
        // Preserve the form's explicit course after a remembered login creates a fresh session.
        // This is a requested destination only; canCreate/allows still verify permissions.
        if($type==='context' && $this->param('scopeid')!=='')$context=$this->param('scopeid');
    ...
    private function savesubtask(){if(!$this->validPost())return $this->index('','The form could not be verified. Please try again.');$task=$this->tasks->one($this->id('taskid'));$title=$this->text('title');if(!$task||$title===''||!$this->service->board($task['boardid'],'edit'))return $this->forbidden();$saved=$this->subtasks->createSubtask($task['id'],$title,time());return $saved!==false?$this->index('Subtask added.'):$this->index('','The subtask could not be saved. Your text is kept.');}
