...
class dbhotelscomments extends dbtable
{

    public function init($tableName = null, $pearDb = null, $errorCallback = 'globalPearErrorHandler')
    {
        parent::init('tbl_hotels_storycomments');
    ...
    public function getStoryComments($storyId)
    {
        ...
        if ((is_countable($comments) ? count($comments) : 0) == 0) {
            ...
            $header->type = 3;
            $header->str = $this->objLanguage->languageText('mod_hotels_usercommentsonstory', 'hotels', 'User Comments on this Hotel');
    ...
    public function commentsForm($id)
    {
        ...
        $form->addToForm('<h3>'.$this->objLanguage->languageText('mod_hotels_commentonstory', 'hotels', 'Comment on this Hotel').':</h3>'.$table->show());
