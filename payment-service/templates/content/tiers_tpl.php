...
<section class="payment-workbench membership-plans" aria-labelledby="membership-plans-title">
 ...
 <?php foreach($labels as $code=>$label):$content=$tierContent[$code];$products=$tierProducts[$code]??array();$isCurrent=$tierIsLoggedIn&&$tierEffective===$code;$isIncluded=$tierIsLoggedIn&&($ranks[$code]??0)<$currentRank;?>
  <article class="membership-plan<?=$isCurrent?' membership-plan--current':''?>">
   ...
   <div class="membership-plan__price"><?php if($code==='free'):?>No membership fee<?php elseif(!$products):?>Contact us for pricing<?php else:?><?php foreach($products as $product):$price=$product['current_price'];$period=(string)($product['billing_period']??'monthly');?><span><?=$e($money($price))?> <small><?=$e($language->languageText('mod_payment_service_price_'.$period,'payment-service'))?></small><?php if($period==='annual'):?><small class="membership-plan__price-note"><?=$e($money(array('amount_minor'=>(int)round(((int)$price['amount_minor'])/12),'currency'=>$price['currency'])))?> <?=$e($language->languageText('mod_payment_service_annual_price_note','payment-service'))?></small><?php endif;?></span><?php endforeach;?><?php endif;?></div><p><?=$e($content['summary'])?></p></div>
