...
<section class="payment-workbench membership-plans" aria-labelledby="membership-plans-title">
 <header><p class="eyebrow">MEMBERSHIP</p><h1 id="membership-plans-title">Choose how you want to learn</h1><p>Compare membership levels and explore the courses available at each level. Your current membership is <strong><?=$e($labels[$tierEffective]??'Free')?></strong>.</p></header>
 ...
 <?php foreach($labels as $code=>$label):$content=$tierContent[$code];$products=$tierProducts[$code]??array();$isCurrent=$tierEffective===$code;$isIncluded=($ranks[$code]??0)<$currentRank;?>
  <article class="membership-plan<?=$isCurrent?' membership-plan--current':''?>">
   ...
   <div class="membership-plan__price"><?php if($code==='free'):?>No membership fee<?php elseif(!$products):?>Contact us for pricing<?php else:?><?php foreach($products as $product):$price=$product['current_price'];?><span><?=$e($money($price))?> <small>per <?=($product['billing_period']??'monthly')==='annual'?'year':'month'?></small></span><?php endforeach;?><?php endif;?></div><p><?=$e($content['summary'])?></p></div>
   ...
   <div class="membership-plan__actions"><a class="button chisimba-button-secondary" href="<?=$this->uri(array('action'=>'catalogue','access'=>$code),'context')?>">View <?=$e($label)?> courses</a>
   <?php if(!$isCurrent&&!$isIncluded&&$code!=='free'&&$products):?><a class="button" href="<?=$this->uri(array('action'=>'catalogue','purpose'=>'membership','tier'=>$code),'payment-service')?>">Upgrade to <?=$e($label)?></a><?php endif;?></div>
  </article>
 <?php endforeach;?></div>
