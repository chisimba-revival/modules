...
$checks = array(
    ...
    'AI drafts survive lecturer mark edits' => !str_contains(substr($controller, strpos($controller, 'private function __savestudentmark'), 1200), 'deleteForResult('),
    'stale AI drafts are removed when a student submission is reopened' => str_contains($controller, 'reopenSubmission($resultId)') && str_contains($controller, 'deleteForResult($resultId)'),
    'completed AI drafts can be recovered' => str_contains($jobs, 'function getLatestCompletedForResult') && str_contains($template, 'mod_worksheet_ai_recover'),
    'repeat AI requests reuse the retained draft' => str_contains($controller, 'return $this->nextAction(\'aimarkingjob\', array(\'id\'=>$existingDraft[\'id\']))'),
